...
PYBIND11_MODULE(beagle, m) {
    ...
    py::enum_<BeagleFlags>(m, "BeagleFlags", py::arithmetic(), R"pbdoc(Hardware and implementation capability flags
    ...
    py::enum_<BeagleBenchmarkFlags>(m, "BeagleBenchmarkFlags", py::arithmetic(), R"pbdoc(Benchmarking mode flags for resource performance evaluation with beagleGetOrderedResourceList
    ...
    py::enum_<BeagleOpCodes>(m, "BeagleOpCodes", py::arithmetic(), R"pbdoc(Operation codes
    ...
    m.def("get_version", &beagleGetVersion, R"pbdoc(Get version

    This function returns a pointer to a string with the library version number.
    )pbdoc");


    m.def("get_citation", &beagleGetCitation, R"pbdoc(Get citation
    ...
    m.def("get_resource_list", [](){
        ...
        ,R"pbdoc(Get list of hardware resources
    ...
    m.def("get_benchmarked_resource_list", [](int tipCount,
        ...
        ,R"pbdoc(Get a benchmarked list of hardware resources for the given analysis parameters
    ...
    m.def("finalize_instance", &beagleFinalizeInstance, R"pbdoc(Finalize this instance

    This function finalizes the instance by releasing allocated memory.
    )pbdoc");


    m.def("finalize", &beagleFinalize, R"pbdoc(Finalize the library
    ...
    m.def("set_CPU_thread_count", &beagleSetCPUThreadCount, R"pbdoc(Set number of threads for native CPU implementation
    ...
    ,R"pbdoc(Set the compact state representation for tip node
    ...
    ,R"pbdoc(Set an instance partials buffer for tip node
    ...
    ,R"pbdoc(Set an instance partials buffer
    ...
    ,R"pbdoc(Set a state frequency buffer
    ...
    ,R"pbdoc(Set a category weights buffer
    ...
    ,R"pbdoc(Set the default category rates buffer
    ...
    ,R"pbdoc(Set a category rates buffer
    ...
    ,R"pbdoc(Set pattern partition assignments
    ...
    ,R"pbdoc(Convolve lists of transition probability matrices
    ...
    ,R"pbdoc(Set a finite-time transition probability matrix
    ...
    ,R"pbdoc(Set multiple transition matrices
    ...
    ,R"pbdoc(Calculate or queue for calculation partials using a list of operations
    ...
    ,R"pbdoc(Calculate or queue for calculation partials using a list of partition operations
    ...
    ,R"pbdoc(Block until all calculations that write to the specified partials have completed.
    ...
    m.def("accumulate_scale_factors", [](int instance,
        ...
        ,R"pbdoc(Accumulate scale factors
    ...
    ,R"pbdoc(Accumulate scale factors by partition
    ...
    ,R"pbdoc(Remove scale factors by partition
    ...
    m.def("reset_scale_factors", &beagleResetScaleFactors, R"pbdoc(Reset scalefactors

    This function resets a cumulative scale buffer.
    )pbdoc");


    m.def("reset_scale_factors_by_partition", &beagleResetScaleFactorsByPartition, R"pbdoc(Reset scalefactors by partition

    This function resets a cumulative scale buffer.
    )pbdoc");


    m.def("copy_scale_factors", &beagleCopyScaleFactors, R"pbdoc(Copy scale factors
